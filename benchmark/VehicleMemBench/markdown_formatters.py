...
def md_overview(report_data: dict[BenchMemoryMode, dict[str, Any]]) -> str:
    """生成第1节：总览."""
    lines: list[str] = ["## 1. 总览\n"]
    if not report_data:
        lines.append("无数据。\n")
...
def md_reasoning_cross_comparison(
    report_data: dict[BenchMemoryMode, dict[str, Any]],
) -> str:
    """生成第3节：按推理类型交叉对比."""
...
def md_query_analysis(  # noqa: C901
