...
50 多样化场景（排除极端安全条件）。评价：决策质量、中间阶段质量、延迟 P50/P90、Cohen's d。
...
32 轮交互，4 阶段偏好切换（各 8 轮：高频提醒→静默→视觉详细→混合）。评价：偏好匹配率、权重收敛速度/稳定性、决策分歧度。

## 测试场景

360 维度组合（scenario 4 × fatigue 3 × workload 3 × task_type 5 × has_passengers 2）→ LLM 批量合成 → JSONL 缓存。合成 260 场景，精选 132（安全50 + 多样50 + 个性化32）。
...
- Cohen's κ 函数已定义待集成，校准集/prompt 调整未实现
