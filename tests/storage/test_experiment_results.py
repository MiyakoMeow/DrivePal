...
async def test_read_benchmark_empty(tmp_path, monkeypatch):
    """文件不存在时返回空 dict。"""
    monkeypatch.setattr(
    ...
    assert await read_benchmark() == {}


async def test_read_benchmark_parses(tmp_path, monkeypatch):
    """读取五策略对比数据。"""
    data = {
        "strategies": {
