...
| settings.py | `LLMSettings` dataclass（`load()` / `get_model_group_providers()` / `get_embedding_provider()`），`LLMProviderConfig` dataclass（temperature/concurrency/api_key 配置），`EmbeddingProviderConfig` dataclass（provider 配置）。TOML配置加载（`@cache`，运行时改TOML不生效）。`_resolve_api_key()` — `api_key_env` 指定时读环境变量，缺/空抛 `ValueError`；未指定时读 `api_key` 字段 |
...
| 阈值 | 值 | 位置 |
|------|----|------|
| HTTP read timeout | 120s | _http.py |
