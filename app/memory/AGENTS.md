...
`singleton.py`。线程安全双检锁。`get_memory_module()` 懒初始化，`close_memory_module()` 优雅关闭（释放 store 资源、Flush 未持久化数据）。
...
`observability.py`。`MemoryBankMetrics`(dataclass, 零锁)。字段：search_count、search_latency_ms、search_empty_index_count、search_empty_count、forget_count、forget_removed_count、write_count、write_latency_ms、embedding_latency_ms、background_task_failures、index_load_warnings。方法：snapshot()/reset()。
...
| `memory_bank/config.py` | MemoryBankConfig | 配置参数（pydantic-settings） |
...
`observability.py`。dataclass，零锁。字段列表参见可观测性节。
...
`embedding_client.py`。薄代理。`encode()` 单条编码，`encode_batch()` 批量编码含数量+维度双重校验。重试由 EmbeddingModel 内部处理。
...
| InteractionRecord | id, event_id, query, response, timestamp, memory_strength, last_recall_date | 原始交互 |
| FeedbackData | event_id, action(accept\|ignore) \| None, type, timestamp, modified_content | 用户反馈 |
...
| TransientError | MemoryBankError | 可重试瞬态错误 |
| FatalError | MemoryBankError | 不可恢复永久错误 |
| LLMCallFailedError | TransientError | LLM 调用失败 |
| ConfigError | FatalError | 配置错误（预留未用） |
| IndexIntegrityError | FatalError | FAISS 索引文件损坏（预留未用） |
