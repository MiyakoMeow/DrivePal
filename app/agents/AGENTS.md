...
| joint_decision_done | JointDecision 阶段完成 | `{tasks: {...}}` |
...
**WorkflowStages**（dataclass，3 字段）：各阶段输出快照（context/joint_decision/execution），用于可解释性与调试。
...
| JointDecision | 事件归因 + 策略决策（task_type/confidence/entities/decision） | JSON 含 task_type/confidence/entities/decision |
...
| JointDecisionOutput | `decision` | `strategy`, `reminder_decision` |
