...
`decision` 字段为原始 dict，无 Pydantic 校验，结构由 `JOINT_DECISION_SYSTEM_PROMPT` 定义（`app/agents/prompts.py`）。`decision` 含 `should_remind`（是否提醒）、`timing`（时机）、`is_emergency`、`reminder_content`、`reason` 等子键。`should_remind` 为 LLM 显式输出字段，但可被 `postprocess_decision()`（`app/agents/rules.py`）在 `postpone` 或 `only_urgent` 触发时强制设为 `false`。SSE `decision` 事件（`workflow.py` `run_stream()` L927-933）从 `state.decision["should_remind"]` 和 `state.task["type"]` 提取 `{should_remind, task_type}` 载荷。

## 规则引擎

`app/agents/rules.py`。JointDecision Agent 前执行安全约束，7 条规则数据驱动加载自 `config/rules.toml`。
