...
`app/agents/shortcuts.py`。`ShortcutResolver` 从 `config/shortcuts.toml` 加载预定义模式，匹配高频场景（如"取消提醒""提醒我几点到某地"）不走 LLM 流水线，直接构造 decision dict 送入 Execution。参数解析函数 `parse_time()` / `parse_duration()` 定义于 `agents/pending.py`。
...
`resolve(query)` 返回预构建 decision dict 或 None。返回 None 时走正常三阶段。
...
`app/agents/pending.py`。`PendingReminderManager` 管理非即时触发的三种 timing 模式，另有 `postpone` 布尔标志独立控制是否进入 pending 分支：
...
`postpone` 为 decision dict 中的独立布尔字段，不由 timing 决定。`postpone=true` 时进入 pending 分支，此时 `_map_pending_trigger()` 按 timing → target_time → driving_context.scenario 顺序兜底映射。

`_map_pending_trigger()`（定义于 `workflow.py`）将 decision 映射为 `(trigger_type, trigger_target, trigger_text)`。`location_time` 的 `trigger_target` 内嵌套 `{"location": {lat,lon}, "time": ISO_str}`，拆为两条独立 pending 提醒（一条位置触发 + 一条时间触发）。
...
| 事件 | 触发时机 | data 内容 |
|------|---------|----------|
| stage_start | 每个阶段开始前 | `{stage: "context"\|"joint_decision"\|"execution"}` |
...
| error | 任一阶段失败 | `{code: string, message: string}`，code 取值 `"INTERNAL"` / `"CONTEXT_FAILED"` / `"JOINT_DECISION_FAILED"` / `"EXECUTION_FAILED"` |
...
| task | JointDecision Agent 输出（任务归因部分，可为 None） |
| decision | JointDecision Agent 输出（决策部分，可为 None） |
...
**WorkflowStages**（dataclass，4 字段）：各阶段输出快照（context/task/decision/execution），task 和 decision 均由 JointDecision 阶段填充，用于可解释性与调试。

### Agent 提示词

`app/agents/prompts.py`。两个 Agent（Context + JointDecision）各有系统提示词，均为中文 + JSON 输出，含显式字段名和示例：

| Agent | 职责 | 输出 |
|------|------|------|
| Context | 构建统一上下文（scenario/driver_state/spatial/traffic） | JSON 上下文对象（7 字段，含示例） |
| JointDecision（系统提示词 `JOINT_DECISION_SYSTEM_PROMPT`） | 事件归因 + 策略决策（task_type/confidence/entities/decision） | JSON 含 task_type/confidence/entities/decision |

Execution Agent 无单独提示词——执行逻辑由规则引擎硬约束 + 代码实现。

### 输出模型鲁棒性

`app/agents/workflow.py`。`ContextOutput` 和 `JointDecisionOutput` 使用 Pydantic `Field(validation_alias=AliasChoices(...))` 兜底 LLM 字段名漂移——不同模型/温度下可能产出非标准键名。`extra="forbid"` 保持严格校验，但已知变体通过 alias 自动归一化。
...
| JointDecisionOutput | `decision` | —（无 alias，原始 dict） |
...
`decision` 字段为原始 dict，无 Pydantic 校验，结构由 `JOINT_DECISION_SYSTEM_PROMPT` 定义（`app/agents/prompts.py`）。`decision` 含 `should_remind`（是否提醒，LLM 未输出时为 None）、`timing`（时机）、`is_emergency`、`reminder_content`、`reason` 等子键。`should_remind` 为 LLM 显式输出字段，但可被 `postprocess_decision()`（`app/agents/rules.py`）在 `postpone` 或 `only_urgent` 触发时强制设为 `false`。SSE `decision` 事件（`run_stream()` yield `decision` 事件时）从 `state.decision.get("should_remind")` 和 `state.task.get("type", "general")` 提取 `{should_remind, task_type}` 载荷。

## 规则引擎

`app/agents/rules.py`。`apply_rules()` 共 6 处静态调用：JointDecision 节点 2 处（`_format_constraints_hint()` 生成自然语言约束提示 + 决策前注入 `postpone`）、Execution 节点 4 处（`postprocess_decision()` 强制覆盖决策 + `_check_frequency_guard()` 频次检查 + pending 分支输出路由 + 主分支输出路由）。运行时路径：主路径（JointDecision ×2 + postprocess_decision + frequency_guard + main）最多 5 次；pending 路径（JointDecision ×2 + postprocess_decision + pending）最多 4 次。7 条规则数据驱动加载自 `config/rules.toml`。
...
`load_rules(path: Path)` 从指定路径加载规则，模块级 `SAFETY_RULES` 以 `config/rules.toml` 初始化。加载失败时回退内置 4 条默认规则。条件字段支持 scenario / not_scenario / workload / fatigue_above / has_passengers（AND 组合）。
...
`app/agents/probabilistic.py`。JointDecision Agent 前执行，MemoryBank 启用时自动注入 prompt。

1. **意图推断**（`infer_intent`）：MemoryBank 检索 top-20 相似事件 → 按 type 聚合得分 → 归一化得置信度分布 `{type, intent_confidence, alternative, alt_confidence}`。冷启动（无结果）返回 type='unknown'、低置信度 0.2，无替代意图。
