...
class TestScenario:
    ...
    user_query: str              # 用户输入
    expected_decision: dict      # 期望决策（人工标注）
...
| city_driving + 正常状态 + 各任务 | city_driving_limit | 10 |
...
| SingleLLM | 单次 LLM 调用，prompt 含完整任务描述，直接输出决策 |

**测试场景**：~50 个多样化场景，覆盖所有 scenario × task_type 组合，不含极端安全条件。
...
- **模型**：独立于 Agent 的模型。优先级：环境变量 `JUDGE_MODEL` > `config/llm.toml` 的 `[model_groups.judge]` 配置 > fallback 用模型组 `default`。`JUDGE_MODEL` 格式为模型引用字符串（如 `openrouter/qwen3.5-2b`），对齐现有 `get_judge_model()` 行为。
- **模式**：JSON mode
- **温度**：0.0（最大化一致性）
- **重试**：每场景评 3 次，取中位数
...
- 计算 Judge 与人工的一致率（Cohen's κ 或 exact agreement）
- 若一致率 < 0.7，调整 Judge prompt 或换模型
...
- 不修改现有生产路径
- `AgentWorkflow` 新增可选参数控制消融行为
- 规则引擎跳过：`postprocess_decision()` 增加 `skip_rules` 参数
- 概率推断跳过：`ABLATION_DISABLE_PROB` 内部设置 `PROBABILISTIC_INFERENCE_ENABLED=0`（优先复用已有开关，不新增代码路径）
- 单 LLM：新增独立 prompt + 直接调用 ChatModel（`ABLATION_USE_SINGLE_LLM=1` 时走此路径）
- 反馈关闭：权重读写跳过，始终返回 0.5
