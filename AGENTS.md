...
`experiments/ablation/`。三组消融实验，验证系统各组件的独立贡献。通过 `python -m experiments.ablation` 运行（非 pytest runner）。
...
├── types.py                   # Scenario, Variant, VariantResult, JudgeScores, GroupResult
├── scenario_synthesizer.py    # LLM 批量合成测试场景（360 维度组合 → JSONL 缓存）
├── ablation_runner.py         # 变体调度器（环境变量 set/restore，五变体分发）
├── judge.py                   # LLM-as-Judge 盲评 + 中位数评分（ChatError/JSONDecodeError 双路径兜底）
├── safety_group.py            # 安全性组（Full/-Rules/-Prob，安全合规率）
├── architecture_group.py      # 架构组（Full vs 单LLM，中间阶段评分）
├── personalization_group.py   # 个性化组（20轮4阶段偏好切换，反馈学习）
├── metrics.py                 # 指标计算（Cohen's d，安全合规率，权重收敛）
...
```
uv run python -m experiments.ablation --synthesize-only          # 仅合成场景
uv run python -m experiments.ablation --group safety             # 安全性组
uv run python -m experiments.ablation --group architecture       # 架构组
uv run python -m experiments.ablation --group personalization    # 个性化组
uv run python -m experiments.ablation --group all                # 全部
uv run python -m experiments.ablation --judge-only --group all   # 仅重新评分
```
...
| 组 | 变体 | 场景 | 指标 |
|----|------|------|------|
| **安全性组** | Full / -Rules / -Prob | 50 安全关键场景 | 安全合规率、规则拦截率、决策质量 |
| **架构组** | Full vs 单LLM | 50 多样化场景 | 决策质量、中间阶段评分、延迟 P50/P90 |
| **个性化组** | Full / -Feedback | 20 轮 × 4 阶段 | 偏好匹配率、收敛速度、稳定性、过拟合 |

### 消融开关

通过环境变量控制（`ablation_runner` 在执行前设置，执行后恢复）：

| 变量 | 说明 |
|------|------|
| `ABLATION_DISABLE_RULES` | 1=跳过规则引擎后处理 |
| `ABLATION_DISABLE_PROB` | 内部映射到 `PROBABILISTIC_INFERENCE_ENABLED=0` |
| `ABLATION_DISABLE_FEEDBACK` | 1=反馈权重固定 0.5 |
| `ABLATION_SEED` | 随机种子（默认 42） |
...
| 文件 | 改动 | 理由 |
|------|------|------|
| `app/agents/rules.py` | `postprocess_decision` 返回 `tuple[dict, list[str]]`；`ABLATION_DISABLE_RULES` 检查 | 规则拦截率需观测点埋入流水线 |
| `app/agents/workflow.py` | modifications 四路径传播到 `WorkflowStages.execution`；`_strategy_node` 检查 `ABLATION_DISABLE_FEEDBACK` | 同上；反馈权重需消融开关 |
| `app/agents/prompts.py` | `SINGLE_LLM_SYSTEM_PROMPT` | 架构组单LLM变体用合并提示词 |

### LLM-as-Judge

- **模型**：优先 `JUDGE_MODEL` 环境变量对应模型，否则回退 `[model_groups.default]`
- **盲评**：shuffle 变体输出顺序
- **中位数**：每场景评 3 次取中位数，减少非确定性噪声
- **容错**：`ChatError` → 默认分 3；`JSONDecodeError` → 默认分 3
- **中间阶段评分**：架构组对 Context/Task/Strategy 三阶段输出独立评分

### 单元测试

```
tests/experiments/
├── test_types.py                    # 数据类型验证
├── test_metrics.py                  # 指标计算（Cohen's d 边界）
└── test_scenario_synthesizer.py     # 场景加载/抽样逻辑
```

用 mock，入 CI（`uv run pytest` 自动运行）。12 测试。
