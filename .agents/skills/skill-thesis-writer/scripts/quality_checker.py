...
class QualityChecker:
    ...
    def _load_checklist(self) -> Dict:
        ...
        return {
            ...
            "citation": ["引用格式是否统一", "是否有过度自引", "近5年文献占比是否达标"],
            "writing": [
                "是否存在口语化表达",
                "标点符号使用是否规范",
                "是否存在连续长句",
    ...
    def check_logic(self, text: str) -> List[QualityIssue]:
        ...
        if hypotheses:
            for h in hypotheses:
                ...
                discussion_match = re.search(
                    r"讨论.*" + re.escape(h_text[:10]), text, re.DOTALL
                )
        ...
        # 检查变量定义一致性
        variable_definitions = re.findall(
            r"([\w\u4e00-\u9fa5]+).*?(?:操作性定义|定义为|是指)[：:](.+?)[。\n]", text
        )
    ...
    def check_format(self, text: str) -> List[QualityIssue]:
        ...
        if figures:
            ...
            if max(figure_nums) > 0 and not all(
                i in figure_nums for i in range(1, max(figure_nums) + 1)
            ):
                issues.append(
                    QualityIssue(
                        category="格式",
    ...
    def _generate_summary(self, report: QualityReport) -> str:
        ...
        return (
            f"质量评分：{report.score:.1f}/100（{level}）\n"
            f"发现问题：{report.total_issues}个 "
            f"（错误：{report.errors}，警告：{report.warnings}，提示：{report.infos}）"
        )

    def generate_report(
        self, report: QualityReport, output_format: str = "markdown"
    ) -> str:
        """
        生成格式化的检查报告
        ...
        if report.issues:
            ...
            sorted_issues = sorted(
                report.issues, key=lambda x: severity_order.get(x.severity, 3)
            )
            ...
            for i, issue in enumerate(sorted_issues, 1):
                ...
                lines.append(
                    f"### {i}. {severity_icon} [{issue.severity.upper()}] {issue.category}"
                )
                lines.append("")
