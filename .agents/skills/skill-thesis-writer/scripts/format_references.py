...
class ReferenceFormatter:
    ...
    def detect_duplicates(
        self, refs: Optional[List[Reference]] = None, similarity_threshold: float = 0.85
    ) -> List[Tuple[Reference, Reference, float]]:
        """
        检测重复文献
    ...
    def analyze_timeliness(
        self, refs: Optional[List[Reference]] = None, years_back: int = 5
    ) -> Dict:
        """
        分析文献时效性
    ...
    def sort_references(
        self, refs: Optional[List[Reference]] = None, method: str = "author_year"
    ) -> List[Reference]:
        """
        排序参考文献
        ...
        if method == "author_year":
            return sorted(
                refs, key=lambda r: (r.authors[0] if r.authors else "", r.year)
            )
        elif method == "year_author":
            return sorted(
                refs, key=lambda r: (r.year, r.authors[0] if r.authors else "")
            )
        else:
            return refs
...
def main():
    ...
    parser.add_argument(
        "--check-duplicates", "-c", action="store_true", help="检测重复文献"
    )
    parser.add_argument(
        "--analyze-timeliness", "-a", action="store_true", help="分析文献时效性"
    )
    ...
    if args.analyze_timeliness:
        ...
        print(
            f"近5年文献: {timeliness['recent_count']} ({timeliness['recent_ratio']:.1%})"
        )
