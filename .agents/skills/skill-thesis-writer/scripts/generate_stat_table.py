...
class StatTableGenerator:
    ...
    def _grouped_descriptive(
        self, data: pd.DataFrame, variables: List[str], group_by: str
    ) -> str:
        """分组描述性统计"""
        ...
        for var in variables:
            ...
            for name, group in groups:
                ...
                sd = group[var].std()
                row_data.append(
                    f"{m:.{self.config.decimal_places}f}±{sd:.{self.config.decimal_places}f}"
                )
    ...
    def correlation_matrix(
        ...
        # 表头
        header = (
            "| 变量 | " + " | ".join([f"{i + 1}" for i in range(len(variables))]) + " |"
        )
        rows.append(header)
    ...
    def regression_table(
        self, models: List[Dict], model_names: Optional[List[str]] = None
    ) -> str:
        """
        生成回归分析表
...
def from_statsmodels(results, title: str = "回归分析结果") -> str:
    ...
    rows.append("|------|------|------|------|------|----------|")
    rows.append(
        f"| R² | {results.rsquared:.3f} | 调整R² | {results.rsquared_adj:.3f} | F | {results.fvalue:.3f} |"
    )
    rows.append("")
...
def main():
    ...
    generator = StatTableGenerator(
        TableConfig(title="示例统计表格", note="示例数据，仅用于演示")
    )
