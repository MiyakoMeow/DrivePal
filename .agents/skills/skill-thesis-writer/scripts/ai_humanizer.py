...
class AIHumanizer:
    ...
    DISCIPLINE_EXPRESSIONS = {
        "工科": {
            ...
            "technical": [
                "该模块的输入维度为{}",
                "在GPU环境下，单次推理耗时约{}",
                "模型参数量为{}M，计算量为{}GFlops",
            ],
            "evaluation": [
                "从准确率来看，模型达到了{}%",
                "消融实验表明，移除{}后，{}下降了{}%",
                "与基线模型相比，本文方法在{}上提升了{}个百分点",
            ],
        },
        "心理学": {
            "observation": [
    ...
    def detect(self, text: str) -> DetectionResult:
        ...
        suggestions = self._generate_suggestions(
            cv_score, connective_density, passive_ratio
        )
    ...
    def _calculate_connective_density(self, text: str) -> float:
        ...
        return (
            (connective_count * len(connectives[0])) / total_chars
            if total_chars > 0
            else 0.0
        )
    ...
    def _generate_suggestions(
        self, cv: float, density: float, passive: float
    ) -> List[str]:
        """生成改进建议"""
        suggestions = []
...
def main():
    ...
    parser.add_argument("--detect-only", "-t", action="store_true", help="仅检测不处理")
    parser.add_argument(
        "--aggressive", "-a", action="store_true", help="激进模式（更多改写）"
    )
